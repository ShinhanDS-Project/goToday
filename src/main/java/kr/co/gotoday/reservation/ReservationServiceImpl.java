...
import kr.co.gotoday.payment.PaymentService; // PaymentService 임포트 추가
...
public class ReservationServiceImpl implements ReservationService {
	...
	@Autowired
	PaymentService paymentService; // [추가] 결제 승인을 위해 서비스 주입
	...
	public ReservationVO createReservationWithPaymentent(ReservationVO reservationVO, String paymentKey, String orderId, int amount) {
		...
		try {
			// 1. [수정] 토스페이먼츠 API로 결제 최종 승인 (주석 해제)
			...
			e.printStackTrace();
			// 롤백을 위해 RuntimeException으로 감싸서 던짐
			throw new RuntimeException("예약 및 결제 처리 중 오류가 발생했습니다: " + e.getMessage(), e);
		}
	}
